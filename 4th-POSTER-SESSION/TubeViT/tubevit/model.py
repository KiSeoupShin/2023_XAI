...
from EfficientViT.classification.model.encoder import Encoder
...
#             layers[f"encoder_layer_{i}"] = EncoderBlock( ## 이 EncoderBlock을 EfficientViT로 변경하면 됨
#                 num_heads,
#                 hidden_dim,
#                 mlp_dim,
#                 dropout,
...
class DimensionTransform(nn.Module):
    def __init__(self):
        super(DimensionTransform, self).__init__()
...
class TubeViT(nn.Module):
    ...
    def __init__(
        ...
        self.dimensiontransform = DimensionTransform()

        self.vit = Encoder(num_classes=num_classes)
    ...
    def forward(self, x):
        ...
        x = x + self.pos_embedding ## 이때 x의 shape를 파악해야 함
        x = x.unsqueeze(1)

        x = self.dimensiontransform(x)

        # x = self.encoder(x)  ## 여기를 EfficientViT로 변경 (주의할 점은 embedding 과정은 생략해야 한다는 것)
        x = self.vit(x)

        '''
        두가지 선택지
        1. EfficientViT의 막단을 그대로 사용한다
        2. TubeViT의 막단을 사용한다

        둘 다 AvgPool 하고 Classifier 하는건 동일한 것 같은데 그냥 구현의 차이일 수도
        '''
        # Attention pooling
...
class TubeViTLightningModule(LightningModule):
    '''
    train, inference 시 실제로 불러오는 class
    '''
    def __init__(
        self,
        num_classes,
        ...
        **kwargs,
        ...
        self.save_hyperparameters()  # logs에 있는 yaml 파일과 parameter 저장하는 함수
        super().__init__()
        self.num_classes = num_classes
        self.model = TubeViT(
